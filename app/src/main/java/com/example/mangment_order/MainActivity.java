...
public class MainActivity extends AppCompatActivity implements OrderAdapter.OnOrderListener {
    ...
    private final Handler statusUpdateHandler = new Handler();
    ...
    private void filterOrders(String query) {
        ...
        Set<String>idShipment=new HashSet<>();
        ...
        for (Order order : allOrders) {
            if (lower.isEmpty()
                ...
                idShipment.add(order.getShipmentId());
            }
        }
        // Build filtered shipments list.
        for (Shipment shipment : allShipment) {
            if(idShipment.contains(shipment.getShipmentId()))
     ...
     */
    private void updateStatusInDatabase(Shipment shipment, String newStatus,
                                        Order order, String newStatusOrder) {
        if (shipment.getShipmentId() == null || order.getOrderId() == null) return;

        boolean shipmentAlreadyUpdated = newStatus.equalsIgnoreCase(
                shipment.getStatusOfShipment().toString()
        );
        boolean orderAlreadyUpdated = newStatusOrder.equalsIgnoreCase(
                order.getStatusOfOrder().toString()
        );
        ...
        if (!shipmentAlreadyUpdated) {
            shipmentsRef.child(shipment.getShipmentId())
                    ...
                    .addOnSuccessListener(aVoid -> {
                        // Update local object to prevent repeated calls
                        shipment.setStatusOfShipment(StatusShipment.valueOf(newStatus));
                        Toast.makeText(MainActivity.this,
        ...
        if (!orderAlreadyUpdated) {
            ordersRef.child(order.getOrderId())
                    .child("statusOfOrder")
                    .setValue(newStatusOrder)
                    .addOnSuccessListener(aVoid -> {
                        order.setStatusOfOrder(StatusOrders.valueOf(
                                newStatusOrder.toUpperCase()  // if your enum is uppercase
                        ));
                        Toast.makeText(MainActivity.this,
    ...
    public void onOrderLongClick(int position) {
        ...
        boolean IsRecevied="Received".equalsIgnoreCase(selected.getStatusOfOrder().getStatus());
        popup.setOnMenuItemClickListener(item -> {
            ...
            if ("UpdateOrder".equals(title) || "Update To Complete".equals(title)) {
                ...
                if ("UpdateOrder".equals(title)) {
                    ...
                    selectedShipment.setStatusOfShipment(StatusShipment.DELIVERED);
                    ...
                    shipmentsRef.child(selectedShipment.getShipmentId()).setValue(selectedShipment)
                            .addOnSuccessListener(avoid->{
                                Toast.makeText(MainActivity.this, "Shipment status updated!", Toast.LENGTH_SHORT).show();
                ...
                if(IsRecevied){
                    Toast.makeText(this,"You Can't' Cancel Order after Recevied ",Toast.LENGTH_SHORT).show();
                    return false;
                }
                ...
                if(IsRecevied){
                    Toast.makeText(this,"You Can't' Resume Order after Recevied ",Toast.LENGTH_SHORT).show();
                    return false;
                }
